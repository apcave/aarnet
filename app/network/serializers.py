...
class InterfaceSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if (
            ...
            and "device" in attrs
            and attrs["device"] != self.instance.device
        ):
            raise serializers.ValidationError(
                {
                    "device": (
                        "The device assignment cannot be changed "
                        "after creation."
                    )
                }
            )
    ...
    def update(self, instance, validated_data):
        if (
            ...
            and validated_data["device"] != instance.device
        ):
            raise serializers.ValidationError(
                {
                    "device": (
...
class ConnectionSerializer(serializers.ModelSerializer):
    ...
    start = serializers.PrimaryKeyRelatedField(
    ...
    def validate(self, attrs):
        ...
        if start is None or end is None:
            raise serializers.ValidationError(
                {"detail": "Both start and end interfaces are required."}
            )
    ...
    def _derive_status(self, start, end):
        if (
            start.status == InterfaceStatus.UP
            and end.status == InterfaceStatus.UP
        ):
...
class ConnectionNestedSerializer(serializers.ModelSerializer):
    """Serializer that accepts nested
    network/site/device/interface payloads.
    """
    ...
    def _resolve_network(self, network_data, user):
        ...
        if not title:
            raise serializers.ValidationError(
                {"network": "A network title is required."}
            )
    ...
    def _resolve_site(self, site_data, network, user):
        ...
        if not name:
            raise serializers.ValidationError(
                {"site": "A site name is required."}
            )

        site = Site.objects.filter(
            user=user, network=network, name=name
        ).first()
        if site is None:
    ...
    def _resolve_device(self, device_data, site, user):
        ...
        if not name:
            raise serializers.ValidationError(
                {"device": "A device name is required."}
            )
        ...
        if device is None:
            device = Device.objects.create(
                ...
                site=site,
                name=name,
                serial_number=device_data.get(
                    "serial_number", f"{site.name}-{name}-{user.id}"
                ),
            )
        return device
    ...
    def _resolve_interface(self, interface_data, device, user):
        ...
        if not name:
            raise serializers.ValidationError(
                {"interface": "An interface name is required."}
            )

        interface = Interface.objects.filter(
            user=user, device=device, name=name
        ).first()
    ...
    def _resolve_endpoint(self, endpoint_data, user):
        if not endpoint_data:
            raise serializers.ValidationError(
                {"start": "Connection endpoint data is required."}
            )
        ...
        device = self._resolve_device(
            endpoint_data.get("device", {}), site, user
        )
    ...
    def create(self, validated_data):
        ...
        if start_data is None or end_data is None:
            raise serializers.ValidationError(
                {"start": "Both connection endpoints are required."}
            )
    ...
    def update(self, instance, validated_data):
        ...
        if "start" in validated_data:
            instance.start = self._resolve_endpoint(
                validated_data["start"], user
            )
