...
from core.models import Device, Interface, InterfaceStatus, Network, Site, SiteStatus
...
class DeviceApiTests(TestCase):
    ...
    def test_create_device_with_site(self):
        ...
        res = self.client.post(reverse('network:device-list'), payload, format='json')
    ...
    def test_device_crud_endpoints(self):
        network = Network.objects.create(user=self.user, title='Device CRUD network', description='CRUD network')
        site = Site.objects.create(user=self.user, network=network, name='Device CRUD site', description='Site', status=SiteStatus.ACTIVE)
        create_payload = {'site': site.id, 'name': 'Router-1', 'serial_number': 'SER-CRUD-1'}
        create_res = self.client.post(reverse('network:device-list'), create_payload, format='json')
        ...
        put_res = self.client.put(
            detail_url,
            {'site': site.id, 'name': 'Router-2', 'serial_number': 'SER-CRUD-2'},
            format='json',
        )
        self.assertEqual(put_res.status_code, status.HTTP_200_OK)
        self.assertEqual(put_res.data['name'], 'Router-2')

        patch_res = self.client.patch(detail_url, {'serial_number': 'SER-CRUD-3'}, format='json')
    ...
    def test_device_cannot_be_deleted_while_interfaces_are_attached(self):
        ...
        res = self.client.delete(reverse('network:device-detail', args=[device.id]))
    ...
    def test_device_security_requires_authentication_for_all_methods(self):
        ...
        self.assertEqual(
            unauthenticated_client.get(reverse('network:device-list')).status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
        self.assertEqual(
            unauthenticated_client.post(
                reverse('network:device-list'),
                {'site': site.id, 'name': 'No auth device', 'serial_number': 'SEC-DEV-002'},
                format='json',
            ).status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
        ...
        self.assertEqual(
            unauthenticated_client.put(
                detail_url,
                {'site': site.id, 'name': 'Updated denied device', 'serial_number': 'SEC-DEV-003'},
                format='json',
            ).status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
        self.assertEqual(
            unauthenticated_client.patch(detail_url, {'serial_number': 'SEC-DEV-004'}, format='json').status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
        self.assertEqual(
    ...
    def test_device_name_must_be_unique_per_site(self):
        ...
        detail_url = reverse('network:device-detail', args=[original_device.id])

        create_valid = self.client.post(
            reverse('network:device-list'),
            {'site': site.id, 'name': 'Router-2', 'serial_number': 'DUP-DEV-VALID'},
            format='json',
        )
        ...
        create_duplicate = self.client.post(
            reverse('network:device-list'),
            {'site': site.id, 'name': 'Router-1', 'serial_number': 'DUP-DEV-DUPLICATE'},
            format='json',
        )
        self.assertEqual(create_duplicate.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(Device.objects.filter(site=site, name='Router-1').count(), 1)

        put_valid = self.client.put(
            detail_url,
            {'site': site.id, 'name': 'Router-3', 'serial_number': 'DUP-DEV-UPDATED'},
            format='json',
        )
        ...
        put_duplicate = self.client.put(
            detail_url,
            {'site': site.id, 'name': 'Router-2', 'serial_number': 'DUP-DEV-INVALID-UPDATE'},
            format='json',
        )
        ...
        patch_valid = self.client.patch(detail_url, {'name': 'Router-4'}, format='json')
        self.assertEqual(patch_valid.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_valid.data['name'], 'Router-4')

        patch_duplicate = self.client.patch(detail_url, {'name': 'Router-2'}, format='json')
    ...
    def test_device_serial_number_must_be_unique_across_all_devices(self):
        ...
        create_valid = self.client.post(
            reverse('network:device-list'),
            {'site': second_site.id, 'name': 'Router-serial-2', 'serial_number': 'SERIAL-UNIQUE-002'},
            format='json',
        )
        self.assertEqual(create_valid.status_code, status.HTTP_201_CREATED)
        self.assertEqual(create_valid.data['serial_number'], 'SERIAL-UNIQUE-002')

        create_duplicate = self.client.post(
            reverse('network:device-list'),
            {'site': second_site.id, 'name': 'Router-serial-3', 'serial_number': 'SERIAL-UNIQUE-001'},
            format='json',
        )
        self.assertEqual(create_duplicate.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(Device.objects.filter(serial_number='SERIAL-UNIQUE-001').count(), 1)

        put_valid = self.client.put(
            detail_url,
            {'site': second_site.id, 'name': 'Router-serial-4', 'serial_number': 'SERIAL-UNIQUE-003'},
            format='json',
        )
        self.assertEqual(put_valid.status_code, status.HTTP_200_OK)
        ...
        put_duplicate = self.client.put(
            detail_url,
            {'site': second_site.id, 'name': 'Router-serial-5', 'serial_number': 'SERIAL-UNIQUE-002'},
            format='json',
        )
        self.assertEqual(put_duplicate.status_code, status.HTTP_400_BAD_REQUEST)
        original_device.refresh_from_db()
        ...
        patch_valid = self.client.patch(detail_url, {'serial_number': 'SERIAL-UNIQUE-004'}, format='json')
        self.assertEqual(patch_valid.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_valid.data['serial_number'], 'SERIAL-UNIQUE-004')

        patch_duplicate = self.client.patch(detail_url, {'serial_number': 'SERIAL-UNIQUE-002'}, format='json')
        self.assertEqual(patch_duplicate.status_code, status.HTTP_400_BAD_REQUEST)
        original_device.refresh_from_db()
    ...
    def test_device_invalid_site_is_rejected_on_create_update_and_patch(self):
        ...
        create_success = self.client.post(
            reverse('network:device-list'),
            {'site': valid_site.id, 'name': 'Valid site device', 'serial_number': 'GOOD-SITE-1'},
            format='json',
        )
        ...
        create_res = self.client.post(
            reverse('network:device-list'),
            {'site': 999999, 'name': 'Bad site device', 'serial_number': 'BAD-SITE-1'},
            format='json',
        )
        self.assertEqual(create_res.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertFalse(Device.objects.filter(serial_number='BAD-SITE-1').exists())

        put_success = self.client.put(
            detail_url,
            {'site': valid_site.id, 'name': 'Updated valid device', 'serial_number': 'UPDATED-GOOD-SITE'},
            format='json',
        )
        ...
        put_res = self.client.put(
            detail_url,
            {'site': 999999, 'name': 'Updated device', 'serial_number': 'UPDATED-BAD-SITE'},
            format='json',
        )
        ...
        patch_success = self.client.patch(detail_url, {'site': valid_site.id, 'name': 'Patched valid device'}, format='json')
        ...
        patch_res = self.client.patch(detail_url, {'site': 999999}, format='json')
