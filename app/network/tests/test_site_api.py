...
class SiteApiTests(TestCase):
    ...
    def test_site_crud_endpoints(self):
        network = Network.objects.create(user=self.user, title='Site CRUD network', description='CRUD network')
        create_payload = {
            'network': network.id,
            'name': 'HQ site',
        ...
        create_res = self.client.post(reverse('network:site-list'), create_payload, format='json')
        ...
        patch_res = self.client.patch(detail_url, {'description': 'Patched site description'}, format='json')
        self.assertEqual(patch_res.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_res.data['description'], 'Patched site description')
    ...
    def test_site_name_must_be_unique_per_network(self):
        ...
        self.assertEqual(Site.objects.filter(network=network, name='Duplicate site').count(), 1)
        ...
        patch_valid = self.client.patch(detail_url, {'name': 'Patched site name'}, format='json')
        self.assertEqual(patch_valid.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_valid.data['name'], 'Patched site name')

        patch_duplicate = self.client.patch(detail_url, {'name': 'Unique site'}, format='json')
    ...
    def test_site_status_must_be_one_of_the_allowed_choices(self):
        ...
        self.assertFalse(Site.objects.filter(network=network, name='Invalid status site').exists())
        ...
        self.assertFalse(Site.objects.filter(network=network, name='Null status site').exists())
        ...
        patch_valid = self.client.patch(detail_url, {'status': SiteStatus.ACTIVE}, format='json')
        self.assertEqual(patch_valid.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_valid.data['status'], SiteStatus.ACTIVE)

        patch_invalid_value = self.client.patch(detail_url, {'status': 'suspended'}, format='json')
    ...
    def test_site_cannot_be_deleted_while_devices_are_attached(self):
        ...
        res = self.client.delete(reverse('network:site-detail', args=[site.id]))
    ...
    def test_site_security_requires_authentication_for_all_methods(self):
        ...
        self.assertEqual(
            unauthenticated_client.get(reverse('network:site-list')).status_code,
        ...
        self.assertEqual(
            unauthenticated_client.patch(detail_url, {'description': 'No auth'}, format='json').status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
        self.assertEqual(
