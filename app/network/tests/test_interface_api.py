...
class InterfaceApiTests(TestCase):
    ...
    def test_create_interface_and_connection(self):
        ...
        interface_res = self.client.post(
            reverse("network:interface-list"), interface_payload, format="json"
        )
    ...
    def test_interface_crud_endpoints(self):
        network = Network.objects.create(
            user=self.user,
            title="Interface CRUD network",
            description="CRUD network",
        )
        site = Site.objects.create(
            user=self.user,
            network=network,
            name="Interface CRUD site",
            description="Site",
            status=SiteStatus.ACTIVE,
        )
        device = Device.objects.create(
        ...
        create_res = self.client.post(
            reverse("network:interface-list"), create_payload, format="json"
        )
        ...
        patch_res = self.client.patch(
            detail_url, {"speed": 2500}, format="json"
        )
    ...
    def test_interface_name_must_be_unique_per_device(self):
        ...
        detail_url = reverse(
            "network:interface-detail", args=[original_interface.id]
        )
        ...
        patch_valid = self.client.patch(
            detail_url, {"name": "eth3"}, format="json"
        )
        ...
        patch_duplicate = self.client.patch(
            detail_url, {"name": "eth1"}, format="json"
        )
        self.assertEqual(
    ...
    def test_interface_invalid_device_duplicate_name_rejected(
        self,
    ):
        network = Network.objects.create(
            user=self.user,
        ...
        detail_url = reverse(
            "network:interface-detail", args=[duplicate_interface.id]
        )

        create_success = self.client.post(
            reverse("network:interface-list"),
            {
                "device": valid_device.id,
                "name": "eth2",
                "speed": 1500,
                "status": InterfaceStatus.UP,
            },
        ...
        self.assertEqual(
            create_invalid_device.status_code, status.HTTP_400_BAD_REQUEST
        )
        self.assertFalse(
            Interface.objects.filter(device=valid_device, name="eth3").exists()
        )
        ...
        self.assertEqual(
            Interface.objects.filter(device=valid_device, name="eth0").count(),
            1,
        )
        ...
        self.assertEqual(
            put_invalid_device.status_code, status.HTTP_400_BAD_REQUEST
        )
        ...
        self.assertEqual(
            put_duplicate_name.status_code, status.HTTP_400_BAD_REQUEST
        )
        duplicate_interface.refresh_from_db()
        self.assertEqual(duplicate_interface.name, "eth4")

        patch_success = self.client.patch(
            detail_url, {"name": "eth6"}, format="json"
        )
        self.assertEqual(patch_success.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_success.data["name"], "eth6")

        patch_invalid_device = self.client.patch(
            detail_url, {"device": 999999}, format="json"
        )
        self.assertEqual(
            patch_invalid_device.status_code, status.HTTP_400_BAD_REQUEST
        )
        duplicate_interface.refresh_from_db()
        self.assertEqual(duplicate_interface.name, "eth6")

        patch_duplicate_name = self.client.patch(
            detail_url, {"name": "eth0"}, format="json"
        )
        self.assertEqual(
            patch_duplicate_name.status_code, status.HTTP_400_BAD_REQUEST
        )
        duplicate_interface.refresh_from_db()
        self.assertEqual(duplicate_interface.name, "eth6")
    ...
    def test_interface_device_assignment_is_immutable_after_creation(self):
        ...
        patch_res = self.client.patch(
            detail_url, {"device": replacement_device.id}, format="json"
        )
    ...
    def test_interface_status_must_be_one_of_the_allowed_choices(self):
        ...
        create_valid = self.client.post(
            reverse("network:interface-list"),
            {
                ...
                "name": "eth1",
                "speed": 1500,
                "status": InterfaceStatus.MAINTENANCE,
            },
            format="json",
        )
        self.assertEqual(create_valid.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            create_valid.data["status"], InterfaceStatus.MAINTENANCE
        )
        ...
        self.assertFalse(
            Interface.objects.filter(device=device, name="eth2").exists()
        )
        ...
        self.assertFalse(
            Interface.objects.filter(device=device, name="eth3").exists()
        )
        ...
        patch_valid = self.client.patch(
            detail_url, {"status": InterfaceStatus.MAINTENANCE}, format="json"
        )
        self.assertEqual(patch_valid.status_code, status.HTTP_200_OK)
        self.assertEqual(
            patch_valid.data["status"], InterfaceStatus.MAINTENANCE
        )

        patch_invalid_value = self.client.patch(
            detail_url, {"status": "maintenance-mode"}, format="json"
        )
        self.assertEqual(
    ...
    def test_interface_cannot_be_deleted_when_connections_are_attached(self):
        ...
        res = self.client.delete(
            reverse("network:interface-detail", args=[start_interface.id])
        )
        ...
        self.assertTrue(
            Connection.objects.filter(start=start_interface).exists()
        )
