...
class ConnectionApiTests(TestCase):
    ...
    def test_create_connection_with_nested_network_site_device_interface(self):
        ...
        res = self.client.post(
            reverse("network:connection-full-list"), payload, format="json"
        )
        ...
        self.assertEqual(
            Network.objects.filter(
                user=self.user, title="Core Network"
            ).count(),
            1,
        )
        self.assertEqual(
            Site.objects.filter(user=self.user, name="HQ").count(), 1
        )
        self.assertEqual(
            Device.objects.filter(user=self.user, name="Router-1").count(), 1
        )
        self.assertEqual(
            Interface.objects.filter(user=self.user, name="eth0").count(), 1
        )
        self.assertEqual(
            Interface.objects.filter(user=self.user, name="eth1").count(), 1
        )

    def test_connection_crud_endpoints(self):
        network = Network.objects.create(
            user=self.user,
            title="Connection CRUD network",
            description="CRUD network",
        )
        site = Site.objects.create(
            user=self.user,
            network=network,
            name="Connection CRUD site",
            description="Site",
        ...
        device = Device.objects.create(
            ...
            serial_number="CONN-DEV-1",
        )
        start_interface = Interface.objects.create(
            user=self.user, device=device, name="eth0", speed=1000, status="up"
        )
        end_interface = Interface.objects.create(
            user=self.user, device=device, name="eth1", speed=1000, status="up"
        )

        create_payload = {
            "name": "uplink",
            "status": "connected",
            "start": start_interface.id,
            "end": end_interface.id,
        }
        create_res = self.client.post(
            reverse("network:connection-list"), create_payload, format="json"
        )
        ...
        patch_res = self.client.patch(
            detail_url, {"status": "connected"}, format="json"
        )
    ...
    def test_delete_connection_removes_only_the_connection(self):
        network = Network.objects.create(
            user=self.user, title="Delete Network", description="Delete me"
        )
        site = Site.objects.create(
            user=self.user,
            network=network,
        ...
        self.assertTrue(
            Interface.objects.filter(id=start_interface.id).exists()
        )
    ...
    def test_connection_start_and_end_must_be_unique_pair(self):
        network = Network.objects.create(
            user=self.user,
            title="Duplicate connection network",
            description="Validation network",
        )
        site = Site.objects.create(
            user=self.user,
            network=network,
            name="Duplicate connection site",
            description="Site",
            status=SiteStatus.ACTIVE,
        )
        device = Device.objects.create(
            user=self.user,
            site=site,
            name="Duplicate connection device",
            serial_number="CONN-DUP-001",
        )
        start_interface = Interface.objects.create(
            user=self.user, device=device, name="eth0", speed=1000, status="up"
        )
        ...
        other_device = Device.objects.create(
            user=self.user,
            site=site,
            name="Duplicate connection device 2",
            serial_number="CONN-DUP-002",
        )
        other_start_interface = Interface.objects.create(
            user=self.user,
            device=other_device,
            name="eth0",
            speed=1000,
        ...
        other_end_interface = Interface.objects.create(
            user=self.user,
            device=other_device,
            name="eth1",
            speed=1000,
        ...
        replacement_device = Device.objects.create(
            user=self.user,
            site=site,
            name="Duplicate connection device 3",
            serial_number="CONN-DUP-003",
        )
        replacement_start_interface = Interface.objects.create(
            user=self.user,
        ...
        replacement_end_interface = Interface.objects.create(
            user=self.user,
            device=replacement_device,
            name="eth1",
            speed=1000,
        ...
        create_valid = self.client.post(
            reverse("network:connection-list"),
            {
                ...
                "status": "connected",
                "start": other_start_interface.id,
                "end": other_end_interface.id,
            },
            format="json",
        )
        ...
        self.assertEqual(
            Connection.objects.filter(
                start=start_interface, end=end_interface
            ).count(),
            1,
        )
        ...
        put_valid = self.client.put(
            detail_url,
            {
                ...
                "status": "disconnected",
                "start": replacement_start_interface.id,
                "end": replacement_end_interface.id,
            },
            format="json",
        )
        ...
        put_duplicate = self.client.put(
            detail_url,
            {
                "name": "duplicate-connection-update",
                "status": "connected",
                "start": other_start_interface.id,
                "end": other_end_interface.id,
            },
        ...
        patch_valid = self.client.patch(
            detail_url, {"name": "patched-connection"}, format="json"
        )
        ...
        patch_duplicate = self.client.patch(
            detail_url,
            {"start": other_start_interface.id, "end": other_end_interface.id},
            format="json",
        )
        self.assertEqual(
    ...
    def test_connection_status_is_read_only_and_derived_from_interface_states(
        self,
    ):
        network = Network.objects.create(
            user=self.user,
            title="Connection status network",
            description="Validation network",
        )
        site = Site.objects.create(
            user=self.user,
            network=network,
            name="Connection status site",
            description="Site",
            status=SiteStatus.ACTIVE,
        )
        device = Device.objects.create(
            user=self.user,
            site=site,
            name="Connection status device",
            serial_number="CONN-STATUS-001",
        )
        other_device = Device.objects.create(
            user=self.user,
            site=site,
            name="Connection status device 2",
            serial_number="CONN-STATUS-002",
        )
        start_interface = Interface.objects.create(
            user=self.user,
            device=device,
        ...
        alt_start_interface = Interface.objects.create(
            user=self.user,
            device=other_device,
            name="eth0",
            speed=1000,
        ...
        alt_end_interface = Interface.objects.create(
            user=self.user,
            device=other_device,
            name="eth1",
            speed=1000,
        ...
        start_interface.save(update_fields=["status"])
        patch_res = self.client.patch(
            detail_url, {"status": "disconnected"}, format="json"
        )
        self.assertEqual(patch_res.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_res.data["status"], "connected")
        ...
        create_disconnected = self.client.post(
            reverse("network:connection-list"),
            {
                "name": "alt-disconnected",
                "status": "connected",
                "start": alt_start_interface.id,
                "end": alt_end_interface.id,
            },
            format="json",
        )
        self.assertEqual(
            create_disconnected.status_code, status.HTTP_201_CREATED
        )
        self.assertEqual(create_disconnected.data["status"], "disconnected")

    def test_connection_invalid_start_end_rejected_on_create_update_and_patch(
        self,
    ):
        network = Network.objects.create(
            user=self.user,
            title="Connection validation network",
            description="Validation network",
        )
        site = Site.objects.create(
            user=self.user,
            network=network,
            name="Connection validation site",
            description="Site",
            status=SiteStatus.ACTIVE,
        )
        device = Device.objects.create(
            user=self.user,
            site=site,
            name="Connection validation device",
            serial_number="CONN-VALID-001",
        )
        start_interface = Interface.objects.create(
            user=self.user, device=device, name="eth0", speed=1000, status="up"
        )
        ...
        other_device = Device.objects.create(
            user=self.user,
            site=site,
            name="Connection validation device 2",
            serial_number="CONN-VALID-002",
        )
        other_start_interface = Interface.objects.create(
            user=self.user,
            device=other_device,
        ...
        replacement_device = Device.objects.create(
            user=self.user,
            site=site,
            name="Connection validation device 3",
            serial_number="CONN-VALID-003",
        )
        replacement_start_interface = Interface.objects.create(
            user=self.user,
            device=replacement_device,
        ...
        self.assertEqual(
            create_invalid_start.status_code, status.HTTP_400_BAD_REQUEST
        )
        self.assertFalse(
            Connection.objects.filter(name="invalid-start").exists()
        )
        ...
        self.assertEqual(
            create_invalid_end.status_code, status.HTTP_400_BAD_REQUEST
        )
        self.assertFalse(
            Connection.objects.filter(name="invalid-end").exists()
        )
        ...
        self.assertEqual(
            put_invalid_start.status_code, status.HTTP_400_BAD_REQUEST
        )
        connection.refresh_from_db()
        ...
        self.assertEqual(
            put_invalid_end.status_code, status.HTTP_400_BAD_REQUEST
        )
        connection.refresh_from_db()
        self.assertEqual(connection.name, "valid-updated")
        ...
        self.assertEqual(patch_valid.status_code, status.HTTP_200_OK)
        self.assertEqual(
            patch_valid.data["start"], replacement_start_interface.id
        )
        self.assertEqual(patch_valid.data["end"], replacement_end_interface.id)

        patch_invalid_start = self.client.patch(
            detail_url, {"start": 999999}, format="json"
        )
        self.assertEqual(
            patch_invalid_start.status_code, status.HTTP_400_BAD_REQUEST
        )
        connection.refresh_from_db()
        self.assertEqual(connection.start_id, replacement_start_interface.id)

        patch_invalid_end = self.client.patch(
            detail_url, {"end": 999999}, format="json"
        )
        self.assertEqual(
            patch_invalid_end.status_code, status.HTTP_400_BAD_REQUEST
        )
        connection.refresh_from_db()
    ...
    def test_nested_connection_security_auth_for_all_methods(self):
        network = Network.objects.create(
            user=self.user,
            title="Nested security network",
            description="Security network",
        )
        site = Site.objects.create(
            user=self.user,
        ...
        unauthenticated_client = APIClient()
        detail_url = reverse(
            "network:connection-full-detail", args=[connection.id]
        )
