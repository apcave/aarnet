...
class EndpointSecurityTests(TestCase):
    """Ensure all authenticated-only CRUD endpoints reject unauthenticated access."""
    ...
    def assert_401_for_all_methods(self, list_url, detail_url, payload=None, detail_payload=None):
        self.assertEqual(self.client.get(list_url).status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(
            self.client.post(list_url, payload or {}, format='json').status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
        self.assertEqual(self.client.get(detail_url).status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(
            self.client.put(detail_url, detail_payload or payload or {}, format='json').status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
        self.assertEqual(
            self.client.patch(detail_url, detail_payload or payload or {}, format='json').status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
    ...
    def test_network_endpoints_require_authentication_on_all_methods(self):
        ...
        self.assert_401_for_all_methods(
            ...
            payload={'title': 'No auth network', 'description': 'Should fail without auth'},
            detail_payload={'title': 'Updated no auth network', 'description': 'Should fail without auth', 'sites': []},
        )
    ...
    def test_device_endpoints_require_authentication_on_all_methods(self):
        ...
        self.assert_401_for_all_methods(
            ...
            payload={'site': site.id, 'name': 'No auth device', 'serial_number': 'SEC-DEV-002'},
            detail_payload={'site': site.id, 'name': 'Updated no auth device', 'serial_number': 'SEC-DEV-003'},
        )
    ...
    def test_interface_endpoints_require_authentication_on_all_methods(self):
        ...
        self.assert_401_for_all_methods(
            ...
            payload={'device': device.id, 'name': 'eth99', 'speed': 1000, 'status': InterfaceStatus.UP},
            detail_payload={'device': device.id, 'name': 'eth1', 'speed': 2000, 'status': InterfaceStatus.DOWN},
        )
    ...
    def test_connection_endpoints_require_authentication_on_all_methods(self):
        ...
        self.assert_401_for_all_methods(
            ...
            payload={'name': 'no-auth-connection', 'status': 'connected', 'start': start.id, 'end': end.id},
            detail_payload={'name': 'updated-no-auth-connection', 'status': 'disconnected', 'start': start.id, 'end': end.id},
        )

    def test_nested_connection_endpoints_require_authentication_on_all_methods(self):
        network = Network.objects.create(
            user=self.user,
        ...
        self.assert_401_for_all_methods(
            ...
            payload={
                'name': 'nested-no-auth',
                'status': 'connected',
                'start': {'network': {'title': 'N', 'description': 'N desc'}, 'site': {'name': 'A', 'description': 'A desc', 'status': 'active'}, 'device': {'name': 'D1', 'serial_number': 'SER-1'}, 'interface': {'name': 'eth0', 'speed': 1000, 'status': 'up'}},
                'end': {'network': {'title': 'M', 'description': 'M desc'}, 'site': {'name': 'B', 'description': 'B desc', 'status': 'active'}, 'device': {'name': 'D2', 'serial_number': 'SER-2'}, 'interface': {'name': 'eth1', 'speed': 1000, 'status': 'up'}},
            },
            detail_payload={
