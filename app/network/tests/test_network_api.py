...
class NetworkApiTests(TestCase):
    ...
    def test_network_crud_endpoints(self):
        create_payload = {'title': 'CRUD network', 'description': 'Network for CRUD'}
        create_res = self.client.post(reverse('network:network-list'), create_payload, format='json')
        ...
        put_res = self.client.put(
            detail_url,
            {'title': 'Updated network', 'description': 'Updated description', 'sites': []},
            format='json',
        )
        self.assertEqual(put_res.status_code, status.HTTP_200_OK)
        self.assertEqual(put_res.data['title'], 'Updated network')

        patch_res = self.client.patch(detail_url, {'description': 'Patched description'}, format='json')
    ...
    def test_network_title_must_be_unique(self):
        ...
        create_valid = self.client.post(
            ...
            {'title': 'Another valid network', 'description': 'Valid creation path'},
            format='json',
        )
        ...
        create_duplicate = self.client.post(
            reverse('network:network-list'),
            {'title': 'Unique network title', 'description': 'Duplicate title'},
            format='json',
        )
        self.assertEqual(create_duplicate.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(Network.objects.filter(title='Unique network title').count(), 1)

        put_valid = self.client.put(
            detail_url,
            {'title': 'Updated network title', 'description': 'Updated description', 'sites': []},
            format='json',
        )
        ...
        put_duplicate = self.client.put(
            detail_url,
            {'title': 'Another valid network', 'description': 'Should fail duplicate', 'sites': []},
            format='json',
        )
        ...
        patch_valid = self.client.patch(detail_url, {'title': 'Patched network title'}, format='json')
        self.assertEqual(patch_valid.status_code, status.HTTP_200_OK)
        self.assertEqual(patch_valid.data['title'], 'Patched network title')

        patch_duplicate = self.client.patch(detail_url, {'title': 'Another valid network'}, format='json')
    ...
    def test_network_cannot_be_deleted_while_sites_are_attached(self):
        ...
        res = self.client.delete(reverse('network:network-detail', args=[network.id]))
    ...
    def test_network_security_requires_authentication_for_all_methods(self):
        ...
        self.assertEqual(
            unauthenticated_client.get(reverse('network:network-list')).status_code,
        ...
        self.assertEqual(
            unauthenticated_client.put(
                detail_url,
                {'title': 'Updated denied', 'description': 'No auth', 'sites': []},
                format='json',
            ).status_code,
            status.HTTP_401_UNAUTHORIZED,
        )
